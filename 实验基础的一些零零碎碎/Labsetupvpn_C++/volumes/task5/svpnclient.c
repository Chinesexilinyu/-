...
SSL* setupTLSClient(const char* hostname)
{
    ...
    if (SSL_CTX_load_verify_locations(ctx, NULL, CA_DIR) < 1)
    {
        ...
        exit(0);
    }//code explanation 1.1 and 1.2
    ssl = SSL_new(ctx);

    X509_VERIFY_PARAM *vpm = SSL_get0_param(ssl);
    X509_VERIFY_PARAM_set1_host(vpm, hostname, 0);//code explanation 1.3
...
int main(int argc, char *argv[])
{
    ...
    SSL_write(ssl, argv[5], strlen(argv[5])); // local last IP//code explanation3.1
    //printf("please input the username!\n");
    //SSL_write(ssl, argv[5], strlen(argv[5])); // username
    //SSL_write(ssl, getpass(argv[5]), strlen(argv[5])); // password
